...
class BreedsTableViewController: UITableViewController, NSFetchedResultsControllerDelegate {
    ...
    func xml2CD(){
        ...
        let parser = XMLBreedsParser(xmlName: "breeds.xml")
        parser.parsing()
        
        // get the parsed data
        let breedsData = parser.breedsData
        
        // traverse the parse data to make new managed objects
        for dog in breedsData{
            pEntity = NSEntityDescription.entity(forEntityName: "Dog", in: context)
            pMangedObject = Dog(entity: pEntity, insertInto: context)
            
            pMangedObject.breed    = dog.breed
            pMangedObject.traits = dog.traits
            pMangedObject.origin   = dog.origin
            pMangedObject.image   = dog.image
            pMangedObject.url     = dog.url
